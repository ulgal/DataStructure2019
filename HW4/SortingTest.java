...
public class SortingTest
{
	...
	private static int[] DoRadixSort(int[] value)
	{
		...
		int rxValue1[] = new int[value.length];
		int rxValue2[] = new int[value.length];
		for(int i = 0; i < 32; i++){
			if(indexBinary!=0x80000000){
				int indexRx1 = 0;
				int indexRx2 = 0;
				for(int j = 0; j < value.length;  j++){
					if((value[j]&indexBinary)==0){
						rxValue1[indexRx1] = value[j];
						indexRx1++;
					}
					else{
						rxValue2[indexRx2] = value[j];
						indexRx2++;
					}
				}
				indexBinary *= 2;
				//value = rxValue;
				System.arraycopy(rxValue1, 0, value, 0, indexRx1);
				System.arraycopy(rxValue2, 0, value, indexRx1, indexRx2);
			}
			else{
				int indexRx1 = 0;
				int indexRx2 = 0;
